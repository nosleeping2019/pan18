...
def meta_features_extractort(corpus):
    ...
    for paragraph in corpus:
        ...
        ##average number of  titles words ， title words is capital words
...
def baselinesvc(trainx,testx,train_labels,classifier='OneVsRest'):
    # Applying SVM
    max_abs_scaler = preprocessing.MaxAbsScaler()
    scaled_train_data = max_abs_scaler.fit_transform(trainx)
    scaled_test_data = max_abs_scaler.transform(testx)
...
def meta_features():
    ...
    StandardScaler().fit(meta_train)
    StandardScaler().fit(meta_testX)
...
def extract_vocabulary(texts,n,ft):
    ...
    for (text,label) in texts:
        text_occurrences=represent_text(text,n) # if take 1-3 gram, this needs to be modified
...
def countfeature():
    ctv = CountVectorizer(analyzer='word',token_pattern=r'\w{1,}',
                ngram_range=(1, 3), stop_words = eng_stopwords, vocabulary=vocabulary)
    all_ctv = ctv.fit_transform(train_texts)
...
if __name__ == '__main__':
    ...
    for index, problem in enumerate(problems):
        ...
        vocabulary = extract_vocabulary(train_docs, 3, 5) # this is needed to modify if ngram is 1-3
        countvectorizer = CountVectorizer(analyzer='word', token_pattern=r'\w{1,}',
                              ngram_range=(3, 3), stop_words=eng_stopwords, vocabulary=vocabulary)
        train_data = countvectorizer.fit_transform(train_texts)
        ...
        test_data = countvectorizer.transform(test_texts)
        ...
        # predictions, LR_meta_loss = bulid_LR(meta_trainX, meta_validX, meta_trainY, meta_validY, meta_testX)
        # predictions, SVM_meta_loss = bulid_SVM(ctv_trainX, ctv_validX, ctv_trainY, ctv_validY, ctv_testX)
        # predictions, RF_meta_loss = bulid_RF(meta_trainX, meta_validX, meta_trainY, meta_validY, meta_testX)
        # predictions, xgb_meta_loss = bulid_xgb(meta_trainX, meta_validX, meta_trainY, meta_validY, meta_testX)
        predictions = baselinesvc(train_data, test_data, train_labels)

        # predictions = ['candidate000{0:0=2d}'.format(a+1) for a in predictions] # try one of the predictions to evaluate
        ...
        with open(outpath + os.sep + 'answers-' + 'ctv-linearsvc-' + problem + '.json', 'w') as f:
        ...
        print('\t', 'answers saved to file', 'answers-' + 'ctv-linearsvc-' + problem + '.json')
